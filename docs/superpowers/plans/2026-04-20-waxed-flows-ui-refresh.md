...
- `PolarMyFlow/Info.plist` and `PolarMyFlow.xcodeproj` are generated by XcodeGen and are listed in `.gitignore`. When the plan text says "modify Info.plist", make the equivalent edit under `targets.PolarMyFlow.info.properties` in `project.yml` and re-run `xcodegen generate`. Never commit Info.plist or `.xcodeproj` directly.
